...
var (
	colors = []string{
		CRITICAL: logging.ColorSeq(logging.ColorMagenta),
		ERROR:    logging.ColorSeq(logging.ColorRed),
		WARNING:  logging.ColorSeq(logging.ColorYellow),
		NOTICE:   logging.ColorSeq(logging.ColorGreen),
		DEBUG:    logging.ColorSeq(logging.ColorCyan),
	}
	boldcolors = []string{
		CRITICAL: logging.ColorSeqBold(logging.ColorMagenta),
		ERROR:    logging.ColorSeqBold(logging.ColorRed),
		WARNING:  logging.ColorSeqBold(logging.ColorYellow),
		NOTICE:   logging.ColorSeqBold(logging.ColorGreen),
		DEBUG:    logging.ColorSeqBold(logging.ColorCyan),
	}
)
...
func (f *stringFormatter) Format(calldepth int, r *logging.Record, output io.Writer) error {
	for _, part := range f.parts {
		if part.verb == fmtVerbStatic {
			...
			case fmtVerbLongfile, fmtVerbShortfile:
				...
				v = fmt.Sprintf("%s:%d", file, caller.Line)
...
func doFmtVerbLevelColor(layout string, level logging.Level, output io.Writer) {
	if layout == "bold" {
